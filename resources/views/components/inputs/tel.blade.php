 ...
 <label for="{{ $name }}" {{ $attributes->merge(['class' => 'text-gray-500 text-sm font-semibold flex flex-col gap-1 ' . $labelClass]) }}>
   ...
   <span class="relative flex overflow-hidden border-gray-200 rounded-lg shadow-xs">
     ...
     @endif
     <input type="tel" {{ $attributes->merge(['class' => 'w-full py-3 text-black bg-white border-none outline-none focus:outline-none focus:border-none focus:shadow-none focus:ring-transparent ' . $inputClass]) }} placeholder="{{ $placeholder }}"
