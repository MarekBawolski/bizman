<x-app-layout>
  ...
  <x-containers.outer title="Tworzenie nowego elementu" buttonStyle="primary" buttonType="submit" buttonText="Zapisz" >
      <x-containers.inner title="">
        <div class="grid grid-cols-2 gap-4">
          <x-inputs.text name="title" :value="$pricedItem->title" labelClass="col-span-2" label="Tytuł" >
             @error('title')
              <span class="text-xs text-red-700">{{ $message }}</span>
            @enderror
          </x-inputs.text>
          <x-inputs.text name="work_hours" :value="$pricedItem->work_hours" label="Godziny" >
             @error('work_hours')
              <span class="text-xs text-red-700">{{ $message }}</span>
            @enderror
          </x-inputs.text>
 ...
 @error('job_type_id')
              <span class="text-xs text-red-700">{{ $message }}</span>
            @enderror
          <x-inputs.textarea name="content" :value="$pricedItem->content" class="col-span-2" label="Treść" >
            @error('content')
              <span class="text-xs text-red-700">{{ $message }}</span>
            @enderror
          </x-inputs.textarea>
